...
int cliqChainSPNorm(const subProblem &subProb, const std::vector<double> &uEnergy, const std::vector<int> &uOffset, const double tau, double &energy, std::vector<double> &nodeLabMarg)
{
 ...
 if (cliqChain.size() > 1) {
  performSPBwdNorm(cliqChain, subProbNodeMap, subProbNodeOffset, subProbDualVar, uEnergy, uOffset, tau, bwdMargVec, expValMaxVec);

  performSPFwdNorm(cliqChain, subProbNodeMap, subProbNodeOffset, subProbDualVar, bwdMargVec, uEnergy, uOffset, tau, energy, nodeLabMarg, expValMaxVec);
 }
 else {
...
// std::cout<<"performSPFwd: numFactors "<<numFactors<<" curFactorInd  "<<curFactorInd<<" twoFactorInd "<<twoFactorInd<<std::endl;
 ...
 double maxOffsetSum = expValMax;
 double maxOffsetSumFwd = 0;
 double maxOffsetSumBwd = 0;
 ...
 for (int iCliqLab = 0; iCliqLab != nCliqLab; ++iCliqLab) {
  ...
  for (std::vector<int>::iterator iNode = memNode.begin(); iNode != memNode.end(); ++iNode) {
   ...
   nodeLabMarg[subProbNodeOffset[subProbNodeIndex] + varAssign[nodeIndex]] += expVal*curBwdMargVec[twoMargInd];
  } //for nodeOne
#endif
  for (std::vector<int>::iterator sumI = twoSumSet.begin(); sumI != twoSumSet.end(); ++sumI) {
   int subProbNodeIndex = subProbNodeMap.at(memNode[*sumI]);

   nodeLabMarg[subProbNodeOffset[subProbNodeIndex] + varAssign[*sumI]] += expVal*curBwdMargVec[twoMargInd];
...
//  std::cout<<"performSPFwd: iterFactor "<<iterFactor<<std::endl;
  ...
  for (int sumFwdIter = 0; sumFwdIter != iterFactor; ++sumFwdIter) {
   maxOffsetSumFwd += expValMaxFwd[sumFwdIter];
  }

  for (int sumBwdIter = numFactors - 2; sumBwdIter != iterFactor; --sumBwdIter) {
   maxOffsetSumBwd += expValMaxBwd[sumBwdIter];
  }
...
//  std::cout<<"performSPFwd: bwdMargVec size "<<bwdMargVec.size()<<" curFactorInd "<<curFactorInd<<std::endl;
  ...
  for (int iCliqLab = 0; iCliqLab != nCliqLab; ++iCliqLab) {
   ...
   for (std::vector<int>::iterator iNode = memNode.begin(); iNode != memNode.end(); ++iNode) {
    ...
    nodeLabMarg[subProbNodeOffset[subProbNodeIndex] + varAssign[nodeIndex]] += expVal*oneMargVec[oneMargInd]*curBwdMargVec[twoMargInd];
   ...
   for (std::vector<int>::iterator sumI = twoSumSet.begin(); sumI != twoSumSet.end(); ++sumI) {
    ...
    nodeLabMarg[subProbNodeOffset[subProbNodeIndex] + varAssign[*sumI]] += expVal*oneMargVec[oneMargInd]*curBwdMargVec[twoMargInd]*exp(maxOffsetSumFwd + maxOffsetSumBwd + expValMax - maxOffsetSum);
  ...
  expValMaxFwd[iterFactor] = expValSumMax;
...
// std::cout<<"performSPFwd: iFactor offset "<<std::distance(cliqChain.begin(),iFactor)<<std::endl;
 ...
 for (int sumFwdIter = 0; sumFwdIter != numFactors-1; ++sumFwdIter) {
  maxOffsetSumFwd += expValMaxFwd[sumFwdIter];
 }
 ...
 for (int iCliqLab = 0; iCliqLab != nCliqLab; ++iCliqLab) {
  ...
  for (std::vector<int>::iterator sumI = sumSet.begin(); sumI != sumSet.end(); ++sumI) {
   ...
   nodeLabMarg[subProbNodeOffset[subProbNodeIndex] + varAssign[*sumI]] += expVal*oneMargVec[oneMargInd];
  ...
  for (std::vector<int>::iterator nodeI = memNode.begin(); nodeI != memNode.end(); ++nodeI) {
   ...
   nodeLabMarg[subProbNodeOffset[subProbNodeIndex] + varAssign[nodeIndex]] += expVal*oneMargVec[oneMargInd]*exp(maxOffsetSumFwd + expValMax - maxOffsetSum);
 ...
 energy = (1/tau)*log(energyExpSum) + maxOffsetSum;
